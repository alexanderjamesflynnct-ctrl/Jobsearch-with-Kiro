...
// Note: Base URL usually points to the root, but we append the controller paths below
    ...
    <div className="pie-chart-container">
      ...
      <div className="pie-legend">
        {segments.map((s, i) => (
          <div
            key={i}
            className="pie-legend-item pie-legend-clickable"
            onClick={() => handleClick(s.outcome)}
          >
            <span
      ...
      {drilldown && (
        <div
          className="pie-drilldown-overlay"
          onClick={() => setDrilldown(null)}
        >
          <div
            className="pie-drilldown-modal"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="pie-drilldown-header">
              <h3>{drilldown.outcome}</h3>
              <span className="pie-drilldown-count">
                {drilldown.jobs.length} job(s)
              </span>
            </div>
...
export default function Dashboard({ anonymize }) {
  ...
  useEffect(() => {
    // All routes updated to the new Controller-based structure
    Promise.all([
      fetch(`${API}/api/Stats`).then((r) => r.json()),
      fetch(`${API}/api/Kanban`).then((r) => r.json()),
      fetch(`${API}/api/Stats/applied-count`).then((r) => r.json()),
      fetch(`${API}/api/Stats/outcomes`).then((r) => r.json()),
      fetch(`${API}/api/Stats/applied-today`).then((r) => r.json()),
    ])
      .then(([s, k, a, o, t]) => {
        setStats(s);
        setKanban(k);
      ...
      {activeJob && (
        <div className="dash-section">
          ...
          <div className="dash-focus-card">
            ...
            <div className="dash-focus-company">
              {anonymize ? anonFn(activeJob.company) : activeJob.company}
            </div>
            <div className="dash-focus-meta">
              {activeJob.location && <span>{activeJob.location}</span>}
              {activeJob.salary && (
                <span className="dash-focus-salary">{activeJob.salary}</span>
              )}
              {activeJob.is_remote && (
                <span className="badge-remote badge-remote-remote">
                  {activeJob.is_remote}
                </span>
              )}
            </div>
              ...
              <div key={status} className="dash-pipeline-row">
                ...
                <div className="dash-pipeline-bar-bg">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (count / kanban.length) * 100)}%`,
                    }}
                  />
                </div>
        ...
        <div className="dash-section">
          ...
          <div className="dash-outcomes-row">
            ...
            <div className="dash-today-widget">
              <div
                className={`dash-today-face ${appliedToday > 0 ? "happy" : "sad"}`}
              >
                {appliedToday > 0 ? "😊" : "😞"}
              </div>
              <div className="dash-today-text">
                {appliedToday > 0 ? (
                  <>
                    <strong>{appliedToday}</strong> application
                    {appliedToday > 1 ? "s" : ""} today!
                  </>
                ) : (
