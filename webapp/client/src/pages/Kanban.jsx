...
const API = "http://localhost:5300/api/Kanban";
const JOBS_API = "http://localhost:5300/api/Jobs";
...
function Card({ card, onMove, onSave, onDragStart, anonymize }) {
  ...
  const loadNotes = async () => {
    const res = await fetch(`${API}/${card.id}/notes`);
  ...
  const deleteNote = async (noteId) => {
    // Matches the C# [HttpDelete("notes/{noteId:int}")]
  ...
  const setActive = async (e) => {
    e.stopPropagation();
    await fetch(`${API}/${card.id}/active`, { method: "POST" });
    if (!card.is_active) {
      setExpanded(true);
  ...
  const loadTimers = async () => {
    const res = await fetch(`${API}/${card.id}/timers`);
  ...
  const deleteCard = async (e) => {
    ...
    // Points to the new JobsController route
    await fetch(`${JOBS_API}/${encodeURIComponent(card.job_listing_id)}`, {
      method: "DELETE",
    });
    onSave();
  };
  ...
  const setFailType = async (failType) => {
    await fetch(`${API}/${card.id}/fail-type`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    >
      <div className="kanban-card-header" onClick={handleExpand}>
        ...
        <div className="kanban-card-badges">
          ...
          {card.is_remote && (
            <span
              className={`badge-remote badge-remote-${card.is_remote.toLowerCase().replace("-", "")}`}
            >
              {card.is_remote}
            </span>
          )}
          {card.fail_type && (
            <span className="badge-fail-type">{card.fail_type}</span>
          )}
          {card.url && (
            <a
              href={card.url}
          ...
          {card.status === "Searched/Found" && (
            <button
              ...
              onClick={setActive}
              title={card.is_active ? "Deselect" : "Mark as focus"}
            >
              {card.is_active ? "★" : "☆"}
            </button>
          )}
        ...
        <div className="kanban-card-body">
                  ...
                  {history.map((h, i) => (
                    <div key={i} className="kanban-history-entry">
                      <div className="kanban-history-move">
                        <span className="kanban-history-from">
                          {h.from_status || "—"}
                        </span>
          ...
          {card.status === "Searched/Found" && (
            <div className="kanban-stopwatch">
              <div className="kanban-stopwatch-header">
                ...
                <span className="kanban-stopwatch-display">
                ...
                <div className="kanban-timers-list">
                  {timers.map((t) => (
                    <div key={t.id} className="kanban-timer-entry">
                      <span className="kanban-timer-duration">
                        {formatDuration(t.duration_seconds)}
                      </span>
                      <span className="kanban-timer-date">
                        {t.created_at.replace("T", " ").replace("Z", " UTC")}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <div className="kanban-move">
            ...
            <select
              ...
              value=""
              onChange={(e) =>
                e.target.value && onMove(card.id, e.target.value)
              }
            >
      ...
      <div className="fail-dialog" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fail-dialog-options">
          {FAIL_TYPES.map((t) => (
            <label
              key={t}
              className={`fail-dialog-option ${selected === t ? "selected" : ""}`}
            >
              <input
                type="radio"
                name="fail_type"
  ...
  return (
    <div className="kanban-page">
        ...
        <div className="kanban-lane-headers-row" ref={headersRef}>
          {LANES.map((lane) => (
            ...
            >
              ...
              <span className="kanban-lane-count">{byLane(lane).length}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {failDialog && (
        <FailTypeDialog
          onConfirm={async (ft) => {
            await moveCard(failDialog.id, failDialog.lane, ft);
            setFailDialog(null);
          }}
