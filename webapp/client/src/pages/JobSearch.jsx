...
export default function JobSearch({ anonymize, setAnonymize }) {
  ...
  // Load search_keywords setting -> GET /api/Settings
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Lookup routes now nested under Jobs controller
    fetch(`${API}/api/Jobs/lookup/countries`)
      .then((r) => r.json())
      .then(setCountries)
      .catch(() => {});
    fetch(`${API}/api/Jobs/lookup/sources`)
      .then((r) => r.json())
      .then(setSources)
      .catch(() => {});
    fetch(`${API}/api/Jobs/lookup/job-types`)
      .then((r) => r.json())
      .then(setJobTypes)
      .catch(() => {});
    fetch(`${API}/api/Jobs/lookup/states`)
      .then((r) => r.json())
      .then(setStates)
      .catch(() => {});
  }, []);
  ...
  const deleteJob = async (id) => {
    // DELETE /api/Jobs/{id}
    await fetch(`${API}/api/Jobs/${encodeURIComponent(id)}`, {
      method: "DELETE",
    });
    fetchJobs(filters, page);
  };
  ...
  const deleteStale = async () => {
    if (
      !window.confirm(
        "Delete all jobs in Searched/Found that have never been moved to another lane?",
      )
    )
      return;
    // DELETE /api/Jobs/stale
    const res = await fetch(`${API}/api/Jobs/stale`, { method: "DELETE" });
    const data = await res.json();
    alert(data.message);
    fetchJobs(filters, page);
    fetch(`${API}/api/Stats`)
  ...
  const runAdzuna = async () => {
    ...
    try {
      // POST /api/Adzuna/run
      const res = await fetch(`${API}/api/Adzuna/run`, {
        method: "POST",
    ...
    <>
      ...
      {showRun && (
        <div
          ...
          onClick={() => !running && setShowRun(false)}
        >
          <div className="run-modal" onClick={(e) => e.stopPropagation()}>
            <h3>Fetch Adzuna Jobs</h3>
            <p className="subtitle">
              Searches all configured countries. Duplicates are automatically
              skipped.
            </p>
            ...
            <div className="run-modal-actions">
              <button
                className="btn-run"
                onClick={runAdzuna}
                disabled={running || !runKeywords.trim()}
              >
            ...
            {running && (
              <div className="run-progress">
                Searching across countries — this may take a minute...
              </div>
            )}
            {runResult && (
              <div
                className={`run-output ${runResult.success ? "run-success" : "run-error"}`}
              >
      ...
      {error && (
        <div className="error-banner">
          ...
          <br />
          <code>dotnet run</code> in <code>api_cs/</code>
        </div>
      )}
      ...
      {loading ? (
        ...
        <>
          ...
          <JobTable
            jobs={jobs}
            onDelete={deleteJob}
            onRefresh={() => fetchJobs(filters, page)}
            anonymize={anonymize}
          />
          {totalPages > 1 && (
