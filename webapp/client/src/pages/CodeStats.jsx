...
const API = "http://localhost:5300/api/CodeStats";
      ...
      <div className="pie-chart-container">
        <div className="pie-chart-svg-wrap">
          <svg viewBox="0 0 200 200" className="pie-chart-svg">
            ...
            >
              {valueKey === "line_count" ? "Lines" : "Files"}
            </text>
          </svg>
        </div>
          ...
          {segments.map((s, i) => (
            <div key={i} className="pie-legend-item">
              ...
              <span className="pie-legend-value">
                {s[valueKey].toLocaleString()} ({(s.pct * 100).toFixed(1)}%)
              </span>
            </div>
          ))}
        </div>
...
export default function CodeStats() {
    ...
    return (
      <div className="code-stats-page">
        ...
        <p className="subtitle">
          No stats available. Click Rescan to generate.
        </p>
      </div>
    );

  const scannedAt = data.breakdown[0]?.scanned_at
    ?.replace("T", " ")
    .replace("Z", " UTC");
  ...
  return (
    <div className="code-stats-page">
      ...
      {scanMsg && <div className="msg success">{scanMsg}</div>}
      <p className="subtitle">
        {data.total_files} files, {data.total_lines.toLocaleString()} total
        lines of code.
